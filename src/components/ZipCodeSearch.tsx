...
import { apiService, combineRiskData, type RiskData } from '../services/api';
...
export function ZipCodeSearch() {
  ...
  const getMockData = (zipCode: string): RiskData[] => {
  ...
  const handleSearch = async () => {
    ...
    try {
      // First, try to get data from the main risk endpoint
      const riskResponse = await apiService.getRiskByZipCode(zipCode);
      setSearchResults(riskResponse.risks);
    } catch (mainError) {
      console.warn('Main API endpoint failed, trying individual endpoints:', mainError);
      
      try {
        // Fallback: try to combine data from individual endpoints
        const combinedData = await combineRiskData(zipCode);
        setSearchResults(combinedData);
      } catch (fallbackError) {
        console.error('All API endpoints failed:', fallbackError);
        
        // Final fallback: use mock data and show warning
        setSearchResults(getMockData(zipCode));
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          ...
          <ScrollFadeSection fadeDirection="left">
            <Card className="p-8 border-0 shadow-xl bg-white">
              ...
              {/* API Status - for development visibility */}
